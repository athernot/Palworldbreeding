"""
Extractor integrity audit - exact replication of extractor logic.
"""
import json
...
print("=" * 60)
print("EXTRACTOR INTEGRITY AUDIT (Exact Extractor Logic)")
print("=" * 60)
...
# Get player container IDs (exact extractor logic)
...
# Collect container GUIDs from player saves (exact extractor logic)
container_guids = set()
...
print(f"   Container GUIDs collected: {len(container_guids)}")
...
print(f"\n2. Owned individual pals (before deduplication): {len(seen)}")
...
# Verify total
total_accounted = ignored_player + ignored_not_owned + len(seen)
...
if len(result) != 176:
    print(f"\nDISCREPANCY: Expected 176, got {len(result)}")
    print(f"Difference: {abs(len(result) - 176)} species")
    
    # Show species in result but not in expected
    print(f"\nFirst 10 species from audit:")
    print(f"   {result[:10]}")
